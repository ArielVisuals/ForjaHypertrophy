  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <div className="group relative">
        <div className="w-10 h-10 rounded-full bg-blue-600/25 border border-blue-500/40 flex items-center justify-center font-black text-blue-200 cursor-pointer hover:border-blue-400/70 transition-all select-none">
          {initial}
        </div>
        ...
        <div className="absolute right-0 top-[120%] w-56 py-3 bg-[#0A0A0B]/90 backdrop-blur-2xl border border-white/20 rounded-[1.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.5),0_0_20px_rgba(59,130,246,0.1)] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-[100] translate-y-2 group-hover:translate-y-0">
          <div className="px-5 py-3 border-b border-white/10 sm:hidden">
            ...
            <p className="text-white/40 text-[10px] uppercase tracking-widest truncate">{user?.email}</p>
          </div>

          <div className="px-2 pt-1">
            <button
              onClick={signOut}
              className="w-full text-left px-4 py-3 text-red-400 hover:bg-red-500/10 rounded-xl transition-all text-sm font-black uppercase tracking-widest flex items-center gap-3 active:scale-95"
            >
